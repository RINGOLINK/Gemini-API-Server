...
> **使用前必读(重要限制与容量规划)**
>
> 1. **上下文长度**:Gemini 网页端点有 **100 万字符**的输入上限,Agent 接入时请将客户端的**最大上下文长度设置为 256K**,超出部分请依赖本项目内置的四级上下文压缩,否则会触发上游拒绝
> 2. **账号容量上限**:本项目最多管理 **27 个**账号窗口;**账号池超过 9 个**时,强烈建议为每个窗口单独配置独立出口 IP(代理),避免同 IP 多账号触发风控
> 3. **积分与账号数量规划**:Pro 账号积分**每 5 小时刷新一次**。Agent 正常开发使用如需 24 小时不间断,建议**至少配置 6 个账号**;如需使用多模态能力(生图/看图等)并确保中间不因积分断流,建议按 **9 个账号**配置;若还需生图、生视频等重度能力,建议配置更多账号
> 4. **工具调用协议**:本项目为 Agent 做了模拟工具协议适配(`tool_calls`),但受网页端模型行为影响,**不保证 100% 成功**,关键任务请保留人工确认环节
